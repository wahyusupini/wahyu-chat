...
def render_chat_time(unique_id):
    ...
    <script>
        (function() {{
            ...
            // Mendeteksi nama singkatan zona waktu di komputer lokal user (WIB/WITA/WIT)
...
if len(st.session_state.messages) == 0:
    with st.chat_message("assistant"):
        st.markdown("Halo! 👋 Saya adalah AI profesional database toko komputer Anda. 💻 Fitur penanda waktu sudah diperbaiki total menggunakan arsitektur Client-Side DOM Injection, dijamin akurat sesuai daerah Anda (WIB/WITA/WIT) ⏰! Silakan ajukan pertanyaan Anda. ✨")
...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        ...
        # Setiap pesan mendapatkan ID unik agar JavaScript tidak bentrok saat merender ulang halaman
